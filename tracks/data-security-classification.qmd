...
```{=html}
<details class="activity"><summary>Activity 3 · Novice — Scan for quasi-identifier risk systematically with Claude Code</summary>
```
...
1. De-identify the table first, exactly as in Activity 2: remove the Name column. Save the remaining five rows (Age, Sex, ZIP, Admission Date, Diagnosis) as `encounters.csv` in an empty working folder.

2. Open Claude Code in that folder and ask it to build the scanner:
...
3. Run the script and read the report. Did it flag the 82-year-old male in ZIP 19104 admitted 2026-03-12 — the row Activity 2 told you to notice? What else did it flag that Activity 2 never mentioned?

4. Now ask Claude Code to test whether generalization fixes it:
...
5. Compare the two reports. Decide, as in Activity 2, whether de-identification alone was enough — but this time with counts, not a hunch.
